...
description: MCPClient authentication — bearer tokens, dynamic JWT, OAuth client credentials, and config fallback.
---

{/* Source: context/concepts/authentication.md — keep in sync */}
...
See [Configuration](/concepts/configuration) for where these values come from.

## Using with MCP Profiles

You can load authentication from [MCP profiles](/concepts/mcp-profiles) and convert it to the auth parameter format:
...
See `examples/auth_examples.py` in the [testmcpy repository](https://github.com/preset-io/testmcpy) for complete working examples of all authentication types.
...
3. **Use profiles for multiple environments**: When you need to manage multiple MCP services or environments (dev, staging, prod). See [MCP Profiles](/concepts/mcp-profiles).

4. **Secure your credentials**: Never hardcode tokens or secrets in your code. Use environment variables or secure config files.

5. **Handle errors gracefully**: Always handle authentication failures when using the MCP client.
...
- [Configuration](/concepts/configuration) - Configuration system and fallback sources
- [MCP Profiles](/concepts/mcp-profiles) - Profile-based multi-environment auth
