...
description: Create custom testmcpy evaluators for domain-specific validation by extending BaseEvaluator.
---

{/* Source: context/guides/custom-evaluators.md — keep in sync */}
...
While testmcpy includes many [built-in evaluators](/concepts/evaluators), you may need custom logic for:
...
Register your evaluator in the factory function in `testmcpy/evals/base_evaluators.py`:
...
<Callout type="info">
This currently requires installing testmcpy from source (`pip install -e .`) so you can edit the factory function. A plugin system for registering evaluators dynamically is planned.
</Callout>
...
- [Evaluators](/concepts/evaluators) — see all built-in evaluators
- [Test Format](/concepts/test-format) — learn the test YAML format
