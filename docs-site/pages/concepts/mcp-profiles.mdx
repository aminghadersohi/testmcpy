---
title: MCP Profiles
description: Manage multiple MCP service environments (dev, staging, prod) with profile-based YAML configuration.
---

{/* Source: context/concepts/mcp-profiles.md — keep in sync */}
...
<Callout type="info">
  Profiles are part of the broader configuration system — see the [Configuration guide](/concepts/configuration) for how profiles interact with other config sources.
</Callout>
...
- **Switch between services** easily with the `--profile` flag
...
See the [Authentication guide](/concepts/authentication) for full details on each auth type.
...
If `${VAR_NAME}` is not set, it resolves to an empty string:
...
Output shows the source of each config value:
...
Note: Currently only `.mcp_services.yaml` is supported. Multiple file support may be added in the future.

## Integration with CI/CD

See the [CI/CD Integration guide](/guides/ci-cd-integration) for complete examples with profiles.

## See Also

- [Configuration](/concepts/configuration) - Full configuration reference
- [Authentication](/concepts/authentication) - Auth types in depth
- [CI/CD Integration](/guides/ci-cd-integration) - GitHub Actions and GitLab CI examples
