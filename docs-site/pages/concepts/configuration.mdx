---
title: Configuration
description: The layered configuration system — config files, environment variables, MCP profiles, and command-line overrides.
---

{/* Source: context/concepts/configuration.md — keep in sync */}
...
Multi-environment configuration with profiles (detailed below, and in [MCP Profiles](/concepts/mcp-profiles)).
...
testmcpy will:
1. Fetch a JWT token from the auth API
2. Cache the token for 50 minutes
...
Manage multiple environments with profile-based configuration. See [MCP Profiles](/concepts/mcp-profiles) for the full reference.
...
Use environment variables for secrets. See the [CI/CD Integration guide](/guides/ci-cd-integration).
...
- [MCP Profiles](/concepts/mcp-profiles) - Detailed profile system docs
