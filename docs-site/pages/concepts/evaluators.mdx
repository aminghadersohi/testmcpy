---
title: Evaluators
description: Complete reference for all built-in evaluators — tool selection, parameters, content, performance — plus custom evaluators.
---

{/* Source: context/concepts/evaluators.md — keep in sync */}

# Evaluator Reference

Complete reference for all available evaluators in testmcpy.

Evaluators inspect the full test transcript — tool calls, parameters, results, the LLM's final response, timing, and cost — and pass or fail each check. All evaluators in a test must pass for the test to pass.
...
- `strict` (optional, default: true): If true, sequence must match exactly (no extra tools). If false, only checks that the sequence appears in order
- `allow_intermediate` (optional, default: false): If true, allows other tools between sequence steps. Only applies when `strict` is false
...
**Use Cases:**
- Validate multi-step workflows (e.g., "generate a chart about births" should first list datasets, then create the chart)
- Ensure the LLM follows logical tool ordering
- Test that the LLM gathers necessary information before performing actions
...
Validates that the generated SQL query is syntactically correct.
...
To create your own evaluators, extend the `BaseEvaluator` class. See the [Custom Evaluators guide](/guides/custom-evaluators) for a full walkthrough.
...
- [Test Format](/concepts/test-format) - How evaluators fit into test files
- [Custom Evaluators](/guides/custom-evaluators) - Build your own evaluators
- [Running Tests](/cli/running-tests) - Running tests from the CLI
